...
# Use Python's standard ``zoneinfo`` module rather than ``pytz`` to avoid an
# external dependency.
...
class FridayVolIndicator:
    ...
    def _window_bounds_utc(self, ref_utc: Optional[datetime] = None):
        ...
        # Ensure we compute for "today" in Chicago.  ``zoneinfo`` uses standard
        # tz-aware datetimes, so we simply attach the time zone with ``replace``.
        start_chi = datetime.combine(now_chi.date(), self.cfg.start_local).replace(tzinfo=CHI)
        end_chi = datetime.combine(now_chi.date(), self.cfg.end_local).replace(tzinfo=CHI)
    ...
    def get_indicator_state(
        ...
        end_local_dt = datetime.combine(now_chi.date(), self.cfg.end_local).replace(tzinfo=CHI)
        ...
        if now_chi < end_local_dt:
            ...
            try:
                ...
                if p_start and p_current and p_start > 0:
                    ...
                    # Signal can be active during monitoring, but status remains MONITORING
        ...
        try:
            # If we've already triggered today for this symbol, return the cached
            # values without re-querying prices to avoid inconsistent mocks.
    ...
    def get_signal_strength(
        ...
        """
        state = self.get_indicator_state(symbol, get_price_at, now_utc)
        # Return the most recent percentage change even if the signal is no
        # longer active (e.g., already triggered earlier in the day).
